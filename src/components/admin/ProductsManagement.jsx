...
export default function ProductsManagement() {
  ...
  // Compress image file to a JPEG dataURL to reduce localStorage size
  ...
  const handleMainImage = (e) => {
    ...
    compressImageFile(file, 1200, 0.8)
      .then((dataUrl) => updateForm({ mainImage: dataUrl }))
      .catch(() => {
  ...
  const handleCarouselImages = (e) => {
    const files = Array.from(e.target.files || []);
    if (!files.length) return;

    const loaders = files.map((file) =>
      compressImageFile(file, 1200, 0.7).catch(() =>
        // fallback to reader
        new Promise((resolve) => {
          const reader = new FileReader();
    ...
    Promise.all(loaders).then((results) => {
      setForm((prev) => ({
        ...prev,
        carouselImages: [...prev.carouselImages, ...results],
      }));
      console.log('[ProductsManagement] compressed carousel images count:', results.length);
    }).catch((err) => {
      console.warn('[ProductsManagement] carousel compression error', err);
    });
  };
  ...
  const handleSubmit = () => {
    ...
    if (category !== "uncategorized") {
      const alreadyExists = shopCategories.some(
    ...
    console.log('[ProductsManagement] submit payload:', { editingId: form.editingId, payload });
    if (form.editingId) {
      updateProduct(form.editingId, payload);
    } else {
      addProduct(payload);
    }
  ...
  const cancelEdit = () => {
    updateForm({ editingId: null, name: '', price: '', originalPrice: '', category: '', customCategory: '', description: '', detailedDescription: '', mainImage: null, carouselImages: [] });
    if (mainImageInput.current) mainImageInput.current.value = "";
    if (carouselInput.current) carouselInput.current.value = "";
  };
  ...
  return (
    <div className="space-y-10 pb-12">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200/70 p-7 md:p-9">
        ...
        <div className="grid md:grid-cols-2 gap-7 mb-10">
          ...
          <div>
            ...
            <div
              className={`relative border-2 border-dashed rounded-xl overflow-hidden transition-all duration-200 cursor-pointer
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-200/60 p-7">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Current Products</h2>
